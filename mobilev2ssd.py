...
class SSD(nn.Module):
    def __init__(self, num_classes, backbone_network,device):
        super(SSD, self).__init__()

        self.device=device
        self.num_classes = num_classes
        self.priors = torch.FloatTensor(priors).to(self.device)
        ...
        if self.backbone_net == 'MobileNetV1':
        	...
        	self.base_net = MobileNetV2_pretrained('mobilenet_v2.pth.tar',self.device).model
        else:
